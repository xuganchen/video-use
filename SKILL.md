...
└── edit/
    ...
    ├── transcripts/<name>.json  ← cached raw ASR JSON (engine in the payload)
...
- A transcription engine. Default `mlx-whisper` is local, free and needs no key — `pip install mlx-whisper` (Apple Silicon). Only `--engine elevenlabs` needs `ELEVENLABS_API_KEY`, either in the environment or in `.env` at the video-use repo root; if missing, ask the user to paste one and write it to `.env` (never to the user's `<videos_dir>`).
